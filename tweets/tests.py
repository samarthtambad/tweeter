...
class TweetTestCase(TestCase):
    def setUp(self):
        ...
        self.userb = User.objects.create_user(username='user2', password='password')
        Tweet.objects.create(content="tweet #1", user=self.user)
        Tweet.objects.create(content="tweet #2", user=self.user)
        Tweet.objects.create(content="tweet #3", user=self.userb)
        self.currentCount = Tweet.objects.all().count()

    def test_tweet_created(self):
        tweet_obj = Tweet.objects.create(content="tweet #4", user=self.user)
